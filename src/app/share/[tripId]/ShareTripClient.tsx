...
export default function ShareTripClient() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className="relative overflow-hidden px-4 pt-10 pb-8" style={{ background: "linear-gradient(180deg,rgba(10,132,255,0.15) 0%,transparent 100%)" }}>
        <div className="max-w-2xl mx-auto">
          <p className="text-[#0A84FF] text-sm font-semibold mb-2 uppercase tracking-wider">Itinerario</p>
          <h1 className="text-white text-3xl font-bold mb-1">
            {trip.destination}{trip.country ? `, ${trip.country}` : ""}
          </h1>
          <p className="text-[#c0c6d6] text-sm mb-6">{formatDateRange(trip.startDate, trip.endDate)}</p>
        ...
        {trip.days.map((day) => (
          <section key={day.dayNumber} className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center text-sm font-bold text-white shrink-0" style={{ background: "linear-gradient(135deg,#0A84FF,#5E5CE6)" }}>
                {day.dayNumber}
              </div>
              <p className="text-white font-semibold text-sm capitalize">{formatDayDate(day.date)}</p>
            </div>
            <div className="space-y-3 pl-4 border-l border-white/[0.06]">
              ...
              {day.activities.map((act) => (
                <div key={act.id} className="bg-[#1a1a2e]/80 border border-white/[0.06] rounded-2xl p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {act.location && <p className="text-[#c0c6d6] text-xs mt-0.5 truncate">📍 {act.location}</p>}
                      <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                        <span className="text-[#0A84FF] text-xs font-medium">{act.time}</span>
                        {act.duration > 0 && <span className="text-[#666] text-xs">·</span>}
                        {act.duration > 0 && (
                      ...
                      {act.description && <p className="text-[#c0c6d6] text-xs mt-2 leading-relaxed line-clamp-3">{act.description}</p>}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 bg-[#0a0a0f]/90 backdrop-blur-xl border-t border-white/[0.06] px-4 py-4">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          <button onClick={handleCopyLink} className="text-[#c0c6d6] text-sm flex items-center gap-1.5 hover:text-white transition-colors">
